...
final class LoadFeedFromRemoteUseCaseTests: XCTestCase {
    ...
    func test_load_deliversErrorOnNon200HTTPResponse() {
        let (sut, client) = makeSUT()
        let sample = [199, 201, 300, 400, 500].enumerated()
            
        sample.forEach { index, code in
    ...
    func test_load_deliversErrorOn200HTTPResponseWithInvalidadJSON() {
        ...
        expect(
            ...
            toCompleteWith: failure(.invalidData),
            when: {
                let invalidJSON = Data("invalidad json".utf8)
    ...
    func test_load_delivbersItemsOn200HTTPResponseWithJSONItems() {
        let (sut, client) = makeSUT()
